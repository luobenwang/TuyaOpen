...
static void create_watch_hands(void)
{
    /* Hour hand - positioned exactly at screen center */
    ...
    /* Minute hand - positioned exactly at screen center */
    ...
    /* Second hand - positioned exactly at screen center */
...
static void update_hands(void)
{
    ...
    float hour_angle = (g_watch.current_time.tm_hour % 12) * 30.0f + 
                      (g_watch.current_time.tm_min / 60.0f) * 30.0f - 90.0f;
    float minute_angle = g_watch.current_time.tm_min * 6.0f - 90.0f;
    float second_angle = g_watch.current_time.tm_sec * 6.0f - 90.0f;
    
    printf("Centered: Hour=%.1f°, Minute=%.1f°, Second=%.1f°\n", 
           hour_angle, minute_angle, second_angle);
    
    /* Update hour hand rotation - pivot at left edge (screen center) */
    lv_obj_set_style_transform_angle(g_watch.hour_hand, (int16_t)(hour_angle * 10), 0);
    lv_obj_set_style_transform_pivot_x(g_watch.hour_hand, 0, 0);  /* Pivot at left edge */
    lv_obj_set_style_transform_pivot_y(g_watch.hour_hand, 3, 0);  /* Pivot at center height */
    
    /* Update minute hand rotation - pivot at left edge (screen center) */
    lv_obj_set_style_transform_angle(g_watch.minute_hand, (int16_t)(minute_angle * 10), 0);
    lv_obj_set_style_transform_pivot_x(g_watch.minute_hand, 0, 0);  /* Pivot at left edge */
    lv_obj_set_style_transform_pivot_y(g_watch.minute_hand, 2, 0);  /* Pivot at center height */
    
    /* Update second hand rotation - pivot at left edge (screen center) */
    lv_obj_set_style_transform_angle(g_watch.second_hand, (int16_t)(second_angle * 10), 0);
    lv_obj_set_style_transform_pivot_x(g_watch.second_hand, 0, 0);  /* Pivot at left edge */
    lv_obj_set_style_transform_pivot_y(g_watch.second_hand, 1, 0);  /* Pivot at center height */
}
