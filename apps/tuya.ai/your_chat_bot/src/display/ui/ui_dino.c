...
#define DINO_JUMP_VELOCITY -12
#define GRAVITY 0.6
#define GROUND_LEVEL 300
#define BASE_OBSTACLE_SPEED 5
#define BASE_OBSTACLE_SPAWN_INTERVAL 60
...
LV_IMG_DECLARE(_dino_alpha_48x48);
...
static void spawn_obstacle(void)
{
    ...
    lv_obj_set_pos(obstacle, 320, y_pos);
...
{
    (void)e;
    if (g_dino.prev_screen) {
        lv_scr_load_anim(g_dino.prev_screen, LV_SCR_LOAD_ANIM_MOVE_RIGHT, 200, 0, false);
    }
}
...
static void on_restart_game(lv_event_t *e)
{
    ...
    // Reset dino position
    lv_obj_set_pos(g_dino.dino, 50, GROUND_LEVEL - 40);
...
static void ui_dino_build_screen(void)
{
    ...
    lv_label_set_text(lbl_back, "Chat");
    lv_obj_center(lbl_back);
    lv_obj_add_event_cb(g_dino.btn_back, on_back_to_chat, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_set_size(g_dino.ground, LV_HOR_RES, 20);
    ...
    lv_img_set_src(g_dino.dino, &_dino_alpha_48x48);
    lv_obj_set_pos(g_dino.dino, 50, GROUND_LEVEL - 40);
    g_dino.dino_y = GROUND_LEVEL - 40;
    ...
    lv_timer_create(game_loop_cb, 16, NULL); // ~60 FPS
}

/* -------------------- Public API -------------------- */
void ui_dino_show(lv_obj_t *prev_screen)
{
    g_dino.prev_screen = prev_screen;
...
void ui_dino_hide(void)
{
    if (g_dino.screen) {
        lv_obj_del(g_dino.screen);
        g_dino.screen = NULL;
    }
    cleanup_obstacles();
}
...
static void ui_dino_build_screen(void)
{
    ...
    // Dino
    g_dino.dino = lv_obj_create(g_dino.screen);
    lv_obj_set_size(g_dino.dino, 30, 40);
    lv_obj_set_style_bg_color(g_dino.dino, lv_color_hex(0x228B22), 0);
    lv_obj_set_style_bg_opa(g_dino.dino, LV_OPA_COVER, 0);
    lv_obj_set_style_radius(g_dino.dino, 6, 0);
    lv_obj_set_pos(g_dino.dino, 50, GROUND_LEVEL - 40);
    g_dino.dino_y = GROUND_LEVEL - 40;
