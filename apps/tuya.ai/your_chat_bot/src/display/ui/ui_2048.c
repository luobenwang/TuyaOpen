...
{
    if (g_2048.prev_screen) {
        lv_scr_load_anim(g_2048.prev_screen, LV_SCR_LOAD_ANIM_MOVE_RIGHT, 200, 0, false);
    }
}
...
static void ui2048_build_screen(void)
{
    ...
    g_2048.score_label = lv_label_create(g_2048.bottom_bar);
    lv_obj_align(g_2048.score_label, LV_ALIGN_LEFT_MID, 8, 0);
    lv_label_set_text(g_2048.score_label, "Score:0");

    g_2048.best_label = lv_label_create(g_2048.bottom_bar);
    lv_obj_align(g_2048.best_label, LV_ALIGN_RIGHT_MID, -8, 0);
    lv_label_set_text(g_2048.best_label, "Best:0");

    // board (use grid layout 4x4)
    const int pad = 10;
    const int top = 60;      // top bar area
    const int bottom = 60;   // bottom bar area
...
void ui_2048_show(lv_obj_t *prev_screen)
{
    g_2048.prev_screen = prev_screen;
